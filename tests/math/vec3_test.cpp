...
TEST(Vec3Test, ParameterConstructor) {
	Vec3<int> vi(1, -1, 0);
	Vec3<float> vf(1.1f, -1.1f, 0.0f);
	Vec3<double> vd(1.1, -1.1, 0.0);
...
TEST(Vec3Test, CopyConstructor) {
	Vec3<int> vi1(1, -1, 0);
	Vec3<float> vf1(1.1f, -1.1f, 0.0f);
	Vec3<double> vd1(1.1, -1.1, 0.0);
...
TEST(Vec3Test, Negative) {
	Vec3<int> vi(-1, -2, -3), vi2(-3, -4, -5);
	Vec3<int> vi3 = vi * vi2;
	ASSERT_EQ(vi3.x, 3);
	ASSERT_EQ(vi3.y, 8);
	ASSERT_EQ(vi3.z, 15);

	Vec3<float> vf(-1.5f, -2.5f, -3.5f), vf2(-3.5f, -4.5f, -5.5f);
	Vec3<float> vf3 = vf - vf2;
	ASSERT_FLOAT_EQ(vf3.x, 2.0f);
	ASSERT_FLOAT_EQ(vf3.y, 2.0f);
	ASSERT_FLOAT_EQ(vf3.z, 2.0f);

	Vec3<double> vd(-1.5, -2.5, -3.5), vd2(-3.5, -4.5, -5.5);
	Vec3<double> vd3 = vd + vd2;
...
TEST(Vec3Test, Assignment) {
	Vec3<int> vi1(1, 1, 1), vi2;
	Vec3<float> vf1(1.1f, 1.1f, 1.1f), vf2;
	Vec3<double> vd1(1.1, 1.1, 1.1), vd2;
...
TEST(Vec3Test, SelfAssignment) {
	Vec3<int> vi(1, 1, 1);
	Vec3<float> vf(1.1f, 1.1f, 1.1f);
	Vec3<double> vd(1.1, 1.1, 1.1);
...
TEST(Vec3Test, AdditionAndSubtraction) {
	Vec3<int> vi1(1, 2, 3), vi2(3, 4, 5);
	Vec3<float> vf1(1.0f, 2.0f, 3.0f), vf2(3.0f, 4.0f, 5.0f);
	Vec3<double> vd1(1.0, 2.0, 3.0), vd2(3.0, 4.0, 5.0);

	Vec3<int> viAdd = vi1 + vi2;
	Vec3<int> viSub = vi1 - vi2;
	Vec3<float> vfAdd = vf1 + vf2;
	Vec3<float> vfSub = vf1 - vf2;
	Vec3<double> vdAdd = vd1 + vd2;
	Vec3<double> vdSub = vd1 - vd2;
...
TEST(Vec3Test, MultiplicationAndDivision) {
	...
	Vec3<int> viMul = vi * 3;
	Vec3<int> viDiv = vi / 2;
	Vec3<float> vfMul = vf * 2.0f;
	Vec3<float> vfDiv = vf / 0.5f;
	Vec3<double> vdMul = vd * 2.0;
	Vec3<double> vdDiv = vd / 2.0;
...
TEST(Vec3Test, ComponentWiseMultiplicationAndDivision) {
	Vec3<int> vi1(2, 3, 4), vi2(4, 5, 6);
	Vec3<float> vf1(1.5f, 2.5f, 3.5f), vf2(2.0f, 3.0f, 4.0f);
	Vec3<double> vd1(1.0, 2.0, 3.0), vd2(2.0, 3.0, 4.0);

	Vec3<int> viMul = vi1 * vi2;
	Vec3<int> viDiv = vi2 / vi1;
	Vec3<float> vfMul = vf1 * vf2;
	Vec3<float> vfDiv = vf1 / vf2;
	Vec3<double> vdMul = vd1 * vd2;
	Vec3<double> vdDiv = vd1 / vd2;
...
TEST(Vec3Test, Compound) {
	...
	vi += Vec3<int>(3, 4, 5);
	vf += Vec3<float>(3.0f, 4.0f, 5.0f);
	vd += Vec3<double>(3.0, 4.0, 5.0);
...
TEST(Vec3Test, Comparison) {
	Vec3<int> vi1(1, 2, 3), vi2(1, 2, 3), vi3(3, 4, 5);
	Vec3<float> vf1(1.0f, 2.0f, 3.0f), vf2(1.0f, 2.0f, 3.0f), vf3(3.0f, 4.0f, 5.0f);
	Vec3<double> vd1(1.0, 2.0, 3.0), vd2(1.0, 2.0, 3.0), vd3(3.0, 4.0, 5.0);
...
TEST(Vec3Test, Length) {
	Vec3<int> vi(3, 4, 5);
	Vec3<float> vf(3.0f, 4.0f, 5.0f);
	Vec3<double> vd(3.0, 4.0, 5.0);
...
TEST(Vec3Test, Normalize) {
	...
	Vec3<double> vd(4.0, 4.0, 2.0);

	Vec3<double> viNorm = vi.normalized();
	Vec3<float> vfNorm = vf.normalized();
	Vec3<double> vdNorm = vd.normalized();
...
TEST(Vec3Test, NormalizeZero) {
	Vec3<int> vi(0, 0, 0);
	Vec3<float> vf(0.0f, 0.0f, 0.0f);
	Vec3<double> vd(0.0, 0.0, 0.0);
...
TEST(Vec3Test, NormalizeTiny) {
	Vec3<float> vf(1e-8f, -1e-8f, 0);
	Vec3<double> vd(1e-16, -1e-16, 0);
...
TEST(Vec3Test, DotProduct) {
	Vec3<int> vi1(1, 2, 3);
	Vec3<int> vi2(3, 4, 5);
	Vec3<float> vf1(1.0f, 2.0f, 3.0f);
	Vec3<float> vf2(3.0f, 4.0f, 5.0f);
	Vec3<double> vd1(1.0, 2.0, 3.0);
	Vec3<double> vd2(3.0, 4.0, 5.0);
